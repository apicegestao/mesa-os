...
begin
 ...
 select proposal.id, proposal.amount, proposal.currency_code, proposal.status, offer.name as offer_name, contact.full_name as payer_name, lower(trim(contact.email)) as payer_email into proposal_record from public.finance_proposals proposal join public.finance_price_versions price on price.id=proposal.price_version_id join public.finance_offers offer on offer.id=price.offer_id join public.crm_opportunities opportunity on opportunity.id=proposal.crm_opportunity_id left join public.crm_contacts contact on contact.id=opportunity.primary_contact_id where proposal.id=target_proposal_id and proposal.status in ('draft','sent','accepted') and (private.has_internal_capability(actor_id,'finance_read_all') or opportunity.owner_identity_id=actor_id);
 ...
 insert into public.finance_provider_checkouts as created(invoice_id,provider_code,external_reference,payer_email) select target_invoice_id,'asaas',invoice.external_reference,proposal_record.payer_email from public.finance_invoices invoice where invoice.id=target_invoice_id returning created.id,created.external_reference into checkout_id,checkout_external_reference;
