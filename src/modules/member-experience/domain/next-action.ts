...
export function deriveNextAction(state: MemberJourneyState): NextAction {
  ...
  if (!state.hasPriority && state.priorityTied) return { eyebrow: "Próxima decisão", title: "Aguardando desempate TutorIA", description: "O diagnóstico encontrou um empate. Nenhuma escolha humana será feita fora da metodologia aprovada.", href: "#prioridade", label: "Entender o resultado" };
  if (!state.hasPriority) return { eyebrow: "Sua prioridade agora", title: "Confirme o foco do ciclo", description: "Revise a menor dimensão do Raio-X e registre por que ela merece atenção neste momento.", href: "#prioridade", label: "Confirmar prioridade" };
