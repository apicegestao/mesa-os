-- TUT-3.8: TutorIA is the autonomous evidence reviewer. Incomplete or
-- uncertain submissions receive an actionable complement request; they never
-- create a human-review queue or require a human approval to progress.
...
declare
  ...
  sequence integer;
begin
  if target_outcome not in ('approved', 'changes_requested') then
    raise exception 'invalid autonomous TutorIA outcome' using errcode = '22023';
  end if;
  if target_confidence < 0 or target_confidence > 1 or char_length(trim(target_rationale)) not between 10 and 2000 then
    raise exception 'invalid TutorIA decision' using errcode = '22023';
  end if;
  if target_outcome = 'approved' and target_confidence < 0.85 then
    raise exception 'auto approval requires high confidence' using errcode = '22023';
  end if;
  ...
  if exists (select 1 from public.evidence_reviews where evidence_id = evidence.id) then
    raise exception 'evidence already decided' using errcode = '23505';
  end if;
  ...
  insert into public.evidence_reviews (
    organization_id, evidence_id, review_sequence, reviewer_kind, outcome,
    confidence, rationale, policy_code, escalation_reason, model_reference
  ) values (
    evidence.organization_id, evidence.id, sequence, 'tutoria', target_outcome,
    target_confidence, trim(target_rationale), 'tut_3_8_autonomous', null,
    nullif(trim(target_model_reference), '')
  );

  if target_outcome = 'approved' then
    update public.missions
      set status = 'completed', completed_at = now(), completed_by = evidence.submitted_by
      where id = mission.id;
