...
function EvolutionIcon() {
  return <svg viewBox="0 0 16 16" aria-hidden="true" focusable="false"><path d="M3 12 7 8l2.5 2.5L14 5" /><path d="M10 5h4v4" /></svg>;
}
...
export function AppChrome({ memberName, logoutAction, progress = 0, activeView = "today", cycleLabel, children, tutoriaWorkbench }: { memberName?: string; logoutAction: () => Promise<void>; progress?: number; activeView?: MemberView; cycleLabel?: string; children: ReactNode; tutoriaWorkbench?: ReactNode }) {
  ...
  return <div className="experience-shell">
    <aside className="experience-sidebar">
      ...
      <nav aria-label="Navegação principal"><Link href="/app" className={activeView === "today" ? "active" : ""}><span>⌂</span>Hoje</Link><Link href="/app?view=journey" className={activeView === "journey" ? "active" : ""}><span>◇</span>Jornada</Link><Link href="/app?view=diagnostics" className={activeView === "diagnostics" ? "active" : ""}><span>◫</span>Diagnósticos</Link><Link href="/app?view=evidence" className={activeView === "evidence" ? "active" : ""}><span>✓</span>Evidências</Link><Link href="/app?view=evolution" className={activeView === "evolution" ? "active" : ""}><span className="nav-icon"><EvolutionIcon /></span>Evolução</Link></nav>
