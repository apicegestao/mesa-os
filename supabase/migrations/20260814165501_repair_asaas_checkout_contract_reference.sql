-- FIN-3.1B repair: qualify local identifiers so PostgreSQL never confuses
-- them with table columns while preparing a provider checkout.
...
declare
  actor_id uuid := auth.uid(); proposal_record record; target_contract_id uuid; target_invoice_id uuid;
  checkout_external_reference text; existing_checkout public.finance_provider_checkouts%rowtype;
begin
  ...
  select proposal.id, proposal.amount, proposal.currency_code, proposal.status, offer.name, contact.full_name, lower(trim(contact.email))
  into proposal_record
  from public.finance_proposals proposal join public.finance_price_versions price on price.id = proposal.price_version_id
  join public.finance_offers offer on offer.id = price.offer_id join public.crm_opportunities opportunity on opportunity.id = proposal.crm_opportunity_id
  left join public.crm_contacts contact on contact.id = opportunity.primary_contact_id
  where proposal.id = target_proposal_id and proposal.status in ('draft', 'sent', 'accepted')
  ...
  insert into public.finance_contracts (proposal_id, status, created_by) values (target_proposal_id, 'draft', actor_id)
  on conflict (proposal_id) do update set updated_at = now() returning id into target_contract_id;
  select checkout.* into existing_checkout from public.finance_provider_checkouts checkout join public.finance_invoices invoice on invoice.id = checkout.invoice_id
  where invoice.contract_id = target_contract_id and checkout.provider_code = 'asaas' for update;
  ...
  select target_invoice_id, 'asaas', invoice.external_reference, proposal_record.payer_email from public.finance_invoices invoice where invoice.id = target_invoice_id
...
revoke all on function public.prepare_asaas_checkout(uuid) from public;
revoke execute on function public.prepare_asaas_checkout(uuid) from anon;
grant execute on function public.prepare_asaas_checkout(uuid) to authenticated;
