...
export function AppChrome({ organizationName, memberName, logoutAction, progress = 0, activeView = "today", children }: { organizationName: string; memberName?: string; logoutAction: () => Promise<void>; progress?: number; activeView?: MemberView; children: ReactNode }) {
  ...
  return <div className="experience-shell">
    <aside className="experience-sidebar">
      ...
      <button className="sidebar-tutoria" type="button" disabled title="Seu contexto já é protegido; a orientação conversacional será ativada em incremento próprio"><span className="mini-orb">T</span><div><strong>Falar com a TutorIA</strong><small>Contexto protegido · orientação em preparação</small></div></button>
    </aside>
    <div className="experience-main">
      ...
      <button className="floating-tutoria" type="button" disabled title="Contexto protegido; orientação conversacional em preparação"><span>T</span><strong>TutorIA</strong></button>
    </div>
  </div>;
}
