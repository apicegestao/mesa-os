...
export function DiagnosticExperience({ initialWorkspace }: { initialWorkspace: DiagnosticWorkspace }) {
  ...
  function persist(onSuccess?: () => void, leaveAfterSave = false) {
    ...
    startTransition(async () => {
      ...
      if (result.ok) {
        onSuccess?.();
        if (leaveAfterSave) router.push("/app");
      }
    });
  }
  ...
  return <div className="diagnostic-overlay" role="presentation"><section ref={overlayRef} className="diagnostic-layout diagnostic-dialog" role="dialog" aria-modal="true" aria-labelledby="diagnostic-title">
    <header className="diagnostic-heading"><div><p className="eyebrow">Raio-X do Empresário · Mês 0</p><h1 id="diagnostic-title">Enxergue a empresa com clareza</h1><p>Responda com base na realidade atual. Seu progresso fica visível em cada pilar.</p></div><div className="diagnostic-heading-actions"><span className="status-pill">Rascunho</span></div></header>
    ...
    <nav className="dimension-nav" aria-label="Dimensões do diagnóstico">
      {workspace.dimensions.map((dimension, index) => {
        ...
        return <button key={dimension.id} className={index === activeIndex ? "active" : ""} aria-current={index === activeIndex ? "step" : undefined} onClick={() => goToDimension(index)}><span>{index + 1}</span><span className="dimension-nav-copy"><strong>{dimension.label}</strong><small>{dimensionCount} de {dimension.questions.length}</small><i className={`dimension-mini-track ${tones[index]}`}><b style={{ width: `${dimensionProgress}%` }} /></i></span></button>;
      })}
    </nav>
    <div className="question-card card" aria-live="polite">
      <div className="active-dimension-heading"><div><p className="dimension-counter">Dimensão {activeIndex + 1} de {workspace.dimensions.length}</p><h2>{activeDimension.label}</h2></div><span>{activeDimension.questions.filter((question) => workspace.answers[question.id] !== undefined).length}/{activeDimension.questions.length}</span></div>
    ...
    <div className="diagnostic-actions">
      <button className="button-secondary" onClick={() => persist(undefined, true)} disabled={isPending}>{isPending ? "Salvando…" : "Salvar e sair depois"}</button>
      {activeIndex > 0 && <button className="button-quiet" onClick={() => goToDimension(activeIndex - 1)}>Anterior</button>}
      {activeIndex < workspace.dimensions.length - 1
        ? <button onClick={() => persist(() => goToDimension(activeIndex + 1))} disabled={isPending || !isDimensionComplete(activeDimension, workspace.answers)}>Salvar e continuar</button>
