...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    log("warn", "finance_request_invalid");
    return NextResponse.json({ error: "invalid_request" }, { status: 400 });
  }
  const supabase = await createSupabaseServerClient();
  const { data: claims } = await supabase.auth.getClaims();
  if (!claims?.claims?.sub) {
    log("warn", "finance_request_forbidden", { action: parsed.data.action });
  ...
  if (result.error || !result.data) {
    log("error", "finance_request_write_failed", { action: action.action, databaseCode: result.error?.code ?? null });
    return NextResponse.json({ error: "unavailable" }, { status: 422 });
  }
  log("info", "finance_request_completed", { action: action.action });
  return NextResponse.json({ id: result.data }, { status: 201, headers: { "Cache-Control": "no-store" } });
}
