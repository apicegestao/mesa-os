...
export function EvidenceOverview({
  ...
  return <section className="records-page" aria-labelledby="evidence-title">
    ...
    <section className="evidence-history proof-history"><div className="records-section-title"><div><p className="eyebrow">Histórico · {cycleLabel ?? "Ciclo atual"}</p><h2>Provas por pilar</h2></div>{implementation?.status === "implemented" && !evidenceSubmitted ? <a className="new-evidence-link" href="/app?view=journey#implementacao">+ Nova evidência</a> : <span>{registered} registro</span>}</div>
      {records.length ? records.map((record) => <article className="proof-row" key={record.id}><span className="proof-icon">▤</span><div><strong>{record.missionTitle}</strong><p>{record.occurredOn}</p><small>{record.description}</small></div><span className="evidence-tag">{record.status === "approved" ? "Aprovada" : record.status === "changes_requested" ? "Correção solicitada" : record.status === "escalated" ? "Em revisão humana" : "Em revisão"}</span><span className="proof-review">{record.reviewerKind === "tutoria" ? "TutorIA" : record.reviewerKind === "human" ? "Revisão humana" : "Registro canônico"}</span><a href="/app?view=journey#implementacao">Abrir →</a></article>) : evidenceSubmitted ? <article className="proof-row"><span className="proof-icon">▤</span><div><strong>{missionTitle ?? "Missão do ciclo"}</strong><p>{pillarLabel ?? "Pilar do ciclo"}</p><small>{implementation?.summary ?? "Implementação comprovada por evidência registrada."}</small></div><span className="evidence-tag">{statusLabel}</span><span className="proof-review">Registro canônico</span><a href="/app?view=journey#implementacao">Abrir →</a></article> : <div className="records-empty"><strong>Nenhuma evidência registrada ainda</strong><p>Quando uma implementação for comprovada, o registro aparecerá aqui com seu estado rastreável.</p>{implementation?.status === "implemented" && <a className="empty-evidence-link" href="/app?view=journey#implementacao">Registrar primeira evidência</a>}</div>}
    </section>
  </section>;
}
