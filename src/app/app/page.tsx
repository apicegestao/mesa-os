...
export default async function AuthenticatedShellPage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  // Keep the protected shell responsive: only load the data required by the
  // active view, and parallelize independent reads. The former implementation
  // loaded every screen plus five workbenches on every navigation.
  const [workspace, mesaOSTerms] = await Promise.all([
    loadDiagnosticWorkspace(supabase, membership.organization_id),
    loadMesaOSTermsState(supabase),
  ]);
  ...
  if (workspace.status !== "completed") {
    const methodologyMap = activeView === "journey" ? await loadPublishedMethodologyMap(supabase) : null;
  ...
  const priority = await loadPriority(supabase, membership.organization_id);
  const cycle = priority ? await loadCycle(supabase, membership.organization_id) : null;
  const needsMissionDetail = activeView !== "diagnostics" && activeView !== "account";
  const [missions, evidenceRecords, measurements, methodologyMap] = await Promise.all([
    cycle && needsMissionDetail ? loadMissions(supabase, cycle.id) : Promise.resolve([]),
    activeView === "evidence" || activeView === "evolution" ? loadEvidenceRecords(supabase, membership.organization_id, cycle?.id) : Promise.resolve([]),
    activeView === "evolution" ? loadMeasurementProjection(supabase, membership.organization_id) : Promise.resolve(undefined),
    activeView === "journey" ? loadPublishedMethodologyMap(supabase) : Promise.resolve(null),
  ]);
  const availableMission = missions.find((mission) => mission.status === "available");
  const toolWorkspace = availableMission && needsMissionDetail ? await loadToolWorkspace(supabase, availableMission.id, availableMission.definition_id) : null;
  ...
  return <AppChrome memberName={memberName} logoutAction={logout} progress={progress} activeView={activeView} cycleLabel={cycle?.title}>
