...
import { buildEvidenceAssessmentPrompt, parseEvidenceAssessment } from "./assessment";
...
describe("TutorIA evidence assessment contract", () => {
  ...
  it("accepts only valid structured output and applies the independent policy", () => {
    ...
    expect(parseEvidenceAssessment(JSON.stringify({ ...safe, confidence: 0.2 }))?.decision).toBe("escalated");
