...
export async function loadTutorIAOrientationContext(supabase: SupabaseClient<Database>, enabled: boolean): Promise<string[]> {
  if (!enabled) return [];
  const { data } = await supabase.from("tutoria_member_memories").select("content,source_kind").eq("state", "active").in("source_kind", ["diagnostic_completed", "priority_confirmed", "cycle_started", "implementation_confirmed", "evidence_approved"]).order("updated_at", { ascending: false }).limit(8);
