...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const emailCodeEnabled = env.NEXT_PUBLIC_EMAIL_CODE_LOGIN_ENABLED;
  const summary = emailCodeEnabled
  ...
  return <main className="shell"><section className="status login-card"><p className="eyebrow">Acesso seguro</p><h1>Entrar no Mesa OS</h1><p className="summary">{summary}</p>{feedback && <p className="feedback feedback-error" role="alert">{feedback}</p>}<LoginForm emailCodeEnabled={emailCodeEnabled} /></section></main>;
}
