...
export function OpsCrmConsole({ concierges, initialWorkspace, isBootstrap, operators, roles }: { concierges: { email: string; identity_id: string }[]; initialWorkspace: CrmWorkspace; isBootstrap: boolean; operators: Operator[]; roles: InternalRole[] }) {
  ...
  async function acceptHandoff(id: string) {
    setPending(true); setMessage(null);
    try { await request("/api/ops/crm/handoffs", { action: "accept", handoffId: id }); setWorkspace((current) => ({ ...current, handoffs: current.handoffs.map((handoff) => handoff.id === id ? { ...handoff, status: "accepted" } : handoff) })); setMessage("Handoff aceito. A matrícula permanece uma ação controlada."); }
    catch { setMessage("Não foi possível aceitar este handoff."); }
    finally { setPending(false); }
  }

  async function updateStage(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); const form = new FormData(event.currentTarget); setPending(true); setMessage(null);
    try { await request("/api/ops/crm/opportunities", { opportunityId: form.get("opportunityId"), stage: form.get("stage"), nextAction: form.get("nextAction"), nextActionDueOn: form.get("nextActionDueOn") || null, lostReason: form.get("lostReason") || null }, "PATCH"); setMessage("Etapa atualizada e auditada. Atualize a página para consultar a carteira consolidada."); }
    catch { setMessage("Não foi possível atualizar esta oportunidade."); }
    finally { setPending(false); }
  }

  async function createHandoff(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); const form = new FormData(event.currentTarget); setPending(true); setMessage(null);
    try { await request("/api/ops/crm/handoffs", { action: "create", opportunityId: form.get("opportunityId"), conciergeIdentityId: form.get("conciergeIdentityId") }); setMessage("Handoff enviado para Concierge e registrado na auditoria."); }
    catch { setMessage("Não foi possível criar este handoff."); }
    finally { setPending(false); }
  }
  ...
  return <section className="ops-console"><header><p className="eyebrow">Backoffice Mesa dos Donos</p><h1>Relacionamento e entrada</h1><p className="summary">CRM comercial e handoff controlado. Não exibe contexto metodológico, conversas ou evolução de membros.</p></header>
    ...
    <section className="status"><h2>Minha carteira</h2>{workspace.opportunities.length ? <ul>{workspace.opportunities.map((opportunity) => <li key={opportunity.id}><strong>{opportunity.title}</strong> · {opportunity.account_name}<br /><span>{opportunity.stage} · Próxima ação: {opportunity.next_action}</span>{canCreate && <form className="auth-form" onSubmit={updateStage}><input type="hidden" name="opportunityId" value={opportunity.id} /><label>Etapa<select name="stage" defaultValue={opportunity.stage}><option value="new">Novo</option><option value="qualified">Qualificado</option><option value="proposal">Proposta</option><option value="negotiation">Negociação</option><option value="won">Ganho</option><option value="lost">Perdido</option></select></label><label>Próxima ação<input name="nextAction" defaultValue={opportunity.next_action} required maxLength={500} /></label><label>Data<input name="nextActionDueOn" type="date" defaultValue={opportunity.next_action_due_on ?? ""} /></label><label>Motivo de perda (se aplicável)<input name="lostReason" maxLength={500} /></label><button type="submit" className="button-quiet" disabled={pending}>Atualizar etapa</button></form>}{canCreate && opportunity.stage === "won" && concierges.length > 0 && <form className="auth-form" onSubmit={createHandoff}><input type="hidden" name="opportunityId" value={opportunity.id} /><label>Encaminhar para Concierge<select name="conciergeIdentityId">{concierges.map((concierge) => <option key={concierge.identity_id} value={concierge.identity_id}>{concierge.email}</option>)}</select></label><button type="submit" className="button-quiet" disabled={pending}>Criar handoff</button></form>}</li>)}</ul> : <p className="feedback">Nenhuma oportunidade disponível para sua função.</p>}</section>
